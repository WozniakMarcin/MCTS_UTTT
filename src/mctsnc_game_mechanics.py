...
def is_action_legal(m, n, board, extra_info, turn, action, legal_actions):
    ...
    is_action_legal_uttt(m, n, board, extra_info, turn, action, legal_actions)   
...
def take_action(m, n, board, extra_info, turn, action):
    ...
    take_action_uttt(m, n, board, extra_info, turn, action)
...
def legal_actions_playout(m, n, board, extra_info, turn, legal_actions_with_count):
    ...
    legal_actions_playout_uttt(m, n, board, extra_info, turn, legal_actions_with_count)   
...
def take_action_playout(m, n, board, extra_info, turn, action, action_ord, legal_actions_with_count):
    ...
    take_action_playout_uttt(m, n, board, extra_info, turn, action, action_ord, legal_actions_with_count)
...
def compute_outcome(m, n, board, extra_info, turn, last_action): # any outcome other than {-1, 0, 1} implies status: game ongoing
    ...
    return compute_outcome_uttt(m, n, board, extra_info, turn, last_action)


    
@cuda.jit(device=True)
def is_action_legal_uttt(m, n, board, extra_info, turn, action, legal_actions):
    i = action // n
    j = action % n
    ...
    if extra_info[9] != -2:
        legal_actions[action] = (board[i, j] == 0) if extra_info[9] == b else False
    else:

        legal_actions[action] = (board[i, j] == 0) if extra_info[b] == -2 else False


@cuda.jit(device=True)
def take_action_uttt(m, n, board, extra_info, turn, action):
    ...
    #Poziomo
    if (board[i0][j0]==board[i0][j0+1]) and (board[i0][j0+1]==board[i0][j0+2]) and board[i0][j0]!= 0:
        extra_info[b] = board[i0][j0]
    if (board[i0+1][j0]==board[i0+1][j0+1]) and (board[i0+1][j0+1]==board[i0+1][j0+2]) and board[i0+1][j0]!= 0:
        extra_info[b] = board[i0+1][j0]
    if (board[i0+2][j0]==board[i0+2][j0+1]) and (board[i0+2][j0+1]==board[i0+2][j0+2]) and board[i0+2][j0]!= 0:
        extra_info[b] = board[i0+2][j0]
    #Pionowo
    if (board[i0][j0]==board[i0+1][j0]) and (board[i0+1][j0]==board[i0+2][j0]) and board[i0][j0]!= 0:
        extra_info[b] = board[i0][j0]
    if (board[i0][j0+1]==board[i0+1][j0+1]) and (board[i0+1][j0+1]==board[i0+2][j0+1]) and board[i0][j0+1]!= 0:
        extra_info[b] = board[i0][j0+1]
    if (board[i0][j0+2]==board[i0+1][j0+2]) and (board[i0+1][j0+2]==board[i0+2][j0+2]) and board[i0][j0+2]!= 0:
        extra_info[b] = board[i0][j0+2]
    #Skosy
    if (board[i0][j0]==board[i0+1][j0+1]) and (board[i0+1][j0+1]==board[i0+2][j0+2]) and board[i0][j0]!= 0:
        extra_info[b] = board[i0][j0]
    if (board[i0][j0+2]==board[i0+1][j0+1]) and (board[i0+1][j0+1]==board[i0+2][j0]) and board[i0][j0+2]!= 0:
        extra_info[b] = board[i0][j0+2]

    sum_of_non_zero=0
    for l in range(3):
        for k in range(3):
            if board[i0+l][j0+k]!=0:
                sum_of_non_zero+=1
...
def legal_actions_playout_uttt(m, n, board, extra_info, turn, legal_actions_with_count):
    ...
    if forced == -2:
        ...
        for b in range(9):
            if extra_info[b] == -2:   # subplansza otwarta
                bi = b // 3
                bj = b % 3
                i0 = bi * 3
                j0 = bj * 3
...
@cuda.jit(device=True)
def take_action_playout_uttt(m, n, board, extra_info, turn, action, action_ord, legal_actions_with_count):
...
@cuda.jit(device=True)
def compute_outcome_uttt(m, n, board, extra_info, turn, last_action):
    ...
    for k in range(3):
        if (extra_info[k] == extra_info[k + 3] and extra_info[k + 3] == extra_info[k + 6]) and extra_info[k] != -2 and extra_info[k] != 0:
            return extra_info[k]

    for k in range(3):
        if (extra_info[k*3] == extra_info[k*3 + 1] and extra_info[k*3 + 1] == extra_info[k*3 + 2]) and extra_info[k*3] != -2 and extra_info[k*3] != 0:
            return extra_info[k*3]
        
    if (extra_info[0] == extra_info[4] and extra_info[4] == extra_info[8]) and extra_info[0] != -2 and extra_info[0] != 0:
        return extra_info[0]
    
    if (extra_info[2] == extra_info[4] and extra_info[4] == extra_info[6]) and extra_info[2] != -2 and extra_info[2] != 0:
        return extra_info[2]
