...
class UTTT(State):
    """
    Class for states of Ultimate Tic-Tac-Toe game.
    ...
    def __init__(self, parent=None):
        """
        Constructor (ordinary or copying) of ``Ultimate Tic-Tac-Toe`` instances - states of Ultimate Tic-Tac-Toe game.
        ...
        if self.parent:
            ...
            self.sub_board_wins = np.copy(self.parent.sub_board_wins)
        else:
            self.board = np.zeros((UTTT.M, UTTT.N), dtype=np.int8)
            self.sub_board_wins = np.zeros(10, dtype=np.int8)

    @staticmethod
    def class_repr():
        """        
        Returns a string representation of class ``UTTT`` (meant to instantiate states of Connect 4 game), informing about the size of board.
        
        Returns:
            str: string representation of class ``UTTT`` (meant to instantiate states of Connect 4 game), informing about the size of board. 
        """
    ...
    def __str__(self):
        ...
        for i in range(UTTT.M + 2):
            s += alphabet[i] + ' '
            for j in range(UTTT.N):
                if i % 4 == 3:
                    if j % 3 == 2 and j != UTTT.N - 1:
                        s += '_  '
                    else:
                        s += '_ '
                else:
    ...
    def take_action_job(self, action_index):
        ...
        """
        j = action_index 
        if self.column_fills[j] == UTTT.M:
            return False
        i = UTTT.M - 1 - self.column_fills[j] 
    ...
    def compute_outcome_job(self):
        ...
        if True: # a bit faster outcome via numba
            ...
            for k in range(1, 4):
                ...
                total += 1
            for k in range(1, 4):
                if i + k >= UTTT.M or j + k >= UTTT.N or self.board[i + k, j + k] != last_token:
                    break
                total += 1            
            if total >= 3:
                return last_token                                    
